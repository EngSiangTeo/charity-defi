...
function getToken(hre: HardhatRuntimeEnvironment, wallet: Wallet) {
  const artifact = hre.artifacts.readArtifactSync("CharityToken");
...
export default async function (hre: HardhatRuntimeEnvironment) {
  ...
  const gasLimit = await erc20.estimateGas.mint(emptyWallet.address, 150000, {
    customData: {
  ...
  await (
    await erc20.mint(emptyWallet.address, 150000, {
